...
@delete_member.args_parser
async def delete_member_parser(bot: cqhttp.Bot, event: cqhttp.Event, state: typing.T_State):
    state[state['_current_key']] = str(event.get_message()).strip()


@delete_member.handle()
async def delete_member_first_receive(bot: cqhttp.Bot, event: cqhttp.Event, state: typing.T_State):
    raw_args = str(event.get_message()).strip()
    if raw_args:
        state['member_id'] = raw_args


@delete_member.got('member_id', prompt=InteractionMessage.REQUEST_ARG)
async def delete_member_requester(bot: cqhttp.Bot, event: cqhttp.Event, state: typing.T_State):
    #   Check if the provided member_id is in the records
    search_result = await data_source.mc.search(state['member_id'])
...
async def update_member_requester(bot: cqhttp.Bot, event: cqhttp.PrivateMessageEvent, state: typing.T_State):
    ...
    #   Check if the provided member_id is in the records
    search_result = await data_source.mc.search(str(state['new_info'][0]).strip())
...
async def search_member_searcher(bot: cqhttp.Bot, event: cqhttp.Event, state: typing.T_State):
    search_result = await data_source.mc.search(str(state['search_info']).strip())
...
async def list_member_retriever(bot: cqhttp.Bot, event: cqhttp.Event, state: typing.T_State):
    member_list_result = await data_source.mc.list()

    if member_list_result['error'] is not None:
        if bot.config.debug:
            await bot.send_private_msg(
                user_id=plugin_config.AUTHOR,
                message=member_list_result['func_info']
            )
            await bot.send_private_msg(
                user_id=plugin_config.AUTHOR,
                message=member_list_result['error']
            )
        await search_member.finish(InteractionMessage.ERROR_MESSAGE)
    else:
        if not member_list_result['response']['result']:
            #   No member stored
            await search_member.finish(InteractionMessage.RECORD_LIST_EMPTY)
        else:
            state['member_list'] = member_list_result['response']['result']
...
def _admin_info_format(raw_info: dict):
    return 'QQ号: {}; '.format(raw_info['member_id']) + \
